...
# Obtaining Jenks natural break points for continuous variables
...
eviction_palette2 = colorBin('Reds', bins = eviction_rate_bins, na.color = '#d9d9d9')
eng_palette2 = colorBin('Blues', bins = pct_eng_bins, na.color = '#d9d9d9')
```
...
leaflet::leaflet(data = census_tracts_crs) %>% 
  addProviderTiles('CartoDB.Positron') %>% 
  addPolygons(
    fillColor = ~eng_palette2(pct_eng),
    fillOpacity = 0.8,
    color = "BDBDC3",
    weight = 1,
    popup = esl_popup,
  ...
  addCircleMarkers(
    lat = ~ census_tracts_crs$lat,
    lng = ~ census_tracts_crs$long,
    color = ~ eviction_palette2(census_tracts_crs$eviction_rate),
