...
# eviction_data = readRDS('./data/joined_data_bklyn')
...
neighborhood_data = neighborhood_data_prep %>% 
  summarize(
    ...
    wt_eviction_filing_rate = round(sum(weighted_eviction_filing_rate), digits = 2),
    wt_pct_nonwhite = round(sum(weighted_pct_nonwhite), digits = 2),
    wt_povertyrate = round(sum(weighted_povertyrate), digits = 2)) %>% 
  ungroup() %>% 
...
neighborhood_data <- within(neighborhood_data, wt_eviction_filing_rate[neighborhood == 'park-cemetery-etc-Brooklyn'] <- NA)
```
...
label_popup3 = paste0(
  "<strong>Neighborhood: </strong>",
  neighborhood_map$ntaname,
  "<br><strong>Eviction filing rate: </strong>",
  neighborhood_map$wt_eviction_filing_rate
)

# get jenks natural breaks
getJenksBreaks(neighborhood_map$wt_eviction_filing_rate, 5)

#set bins based on natural breaks
eviction_filing_rate_bins = c(1.18, 4.94, 9.56, 15.47, 31.10)

#set color palette
eviction_palette3 = colorBin('Reds', bins = eviction_filing_rate_bins, na.color = '#d9d9d9')

```

```{r echo = FALSE, message = FALSE}
#create map of filings
map_filings = leaflet::leaflet(data = neighborhood_map_crs) %>% 
  addProviderTiles('CartoDB.Positron') %>% 
  addPolygons(
    fillColor = ~eviction_palette3(wt_eviction_filing_rate),
    fillOpacity = 0.8,
    color = "BDBDC3",
    weight = 1,
    popup = label_popup3,
    highlightOptions = highlightOptions(color = "black", weight = 2, bringToFront = TRUE)) %>% 
      addLegend('topright',
                pal = eviction_palette3,
                values = ~wt_eviction_filing_rate,
